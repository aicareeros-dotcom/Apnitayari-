...
def _run_web():
    _flask_app.run(
        host="0.0.0.0",
        port=int(_os_flask.environ.get("PORT", 10000))
    )
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                mpd = None
                try:
                    resp = await asyncio.to_thread(requests.get, api_url_call, timeout=30)
                    # parse JSON safely
                    try:
                        data = resp.json()
                    except Exception:
...
for _attempt in range(_max_retries):
    try:
        ...
        print(f"[FloodWait] Telegram ne {_fw.value}s wait maanga. Retry ho raha hai...")
