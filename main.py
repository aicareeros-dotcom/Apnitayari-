...
async def txt_handler(bot: Client, m: Message):  
    ...
    timeout_duration = 3 if auto_flags.get(chat_id) else 5
    try:
    ...
    if "/d" in raw_text7:
        ...
        try:
            ...
            await m.reply_text("❌ <b>Invalid Channel ID!</b>\n<blockquote>Channel ID number hona chahiye, jaise: <code>-1001234567890</code></blockquote>")
            return
    await editable.delete()
    ...
    if channel_id != m.chat.id:
        try:
            ...
            await m.reply_text(
                "❌ <b>Channel Access Error!</b>\n\n"
                "<blockquote>Bot us channel ka member nahi hai ya channel ID galat hai.\n\n"
                "✅ Fix karo:\n"
                "1. Bot ko channel mein Admin banao\n"
                "2. Sahi Channel ID dalo (Ex: <code>-100XXXXXXXXXX</code>)\n"
                "3. /d bhejo agar apne DM mein chahiye</blockquote>"
            )
            return
        except Exception as e:
            await m.reply_text(f"❌ <b>Channel Verify Nahi Hua!</b>\n<blockquote>{e}</blockquote>")
            return
    ...
    try:
        ...
        await m.reply_text("❌ <b>Channel mein send nahi hua!</b>\n\n<blockquote>Bot ko channel mein Admin banao aur dobara try karo.</blockquote>")
        return
    except Exception as e:
